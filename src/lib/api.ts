...
export const statsApi = {
  query: (params: { from: number; to: number; granularity?: 'hour' | 'day'; tz_offset: number; service_key_id?: string }) =>
    request<{ data: StatsRow[]; top_model: TopModel | null }>(
      `/api/stats?from=${params.from}&to=${params.to}&tz_offset=${params.tz_offset}${params.granularity ? `&granularity=${params.granularity}` : ''}${params.service_key_id ? `&service_key_id=${encodeURIComponent(params.service_key_id)}` : ''}`),
};
...
export const requestLogApi = {
  page: (params: { page?: number; page_size?: number; from?: number; to?: number; service_key_id?: string }) =>
