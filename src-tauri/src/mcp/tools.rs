...
impl ServerHandler for XrlMcpTools {
    ...
    async fn list_tools(
        ...
        Ok(ListToolsResult::with_all_items(filter_tools(ws, wf)))
    }
...
fn filter_tools(websearch: bool, webfetch: bool) -> Vec<Tool> {
...
mod tests {
    ...
    fn test_filter_tools_by_switches() {
        assert_eq!(filter_tools(false, false).len(), 0);
        let ws = filter_tools(true, false);
        assert_eq!(ws.len(), 1);
        assert_eq!(ws[0].name.as_ref(), "web_search");
        let wf = filter_tools(false, true);
        assert_eq!(wf.len(), 1);
        assert_eq!(wf[0].name.as_ref(), "web_fetch");
        let both = filter_tools(true, true);
        assert_eq!(both.len(), 2);
        assert_eq!(both[0].name.as_ref(), "web_search");
        assert_eq!(both[1].name.as_ref(), "web_fetch");
    }
